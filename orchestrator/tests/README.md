...
| `test_grounding.py` | The same problem as citations, for NUMBERS. Three synthesis prompts state "no figure that is not in an upstream asset" and nothing checked it, so compliance was per-run luck: a measured run had the analysis asset write that the evidence "does not supply numerical thresholds" for cold starts, and the recommendation built from it advise measuring against "typically 100–1000 ms" — while the same workflow on a second deployment invented nothing. So the matcher is asserted on every near-miss that would make it noisy enough to be switched off (prose counts, `(1)`/`(2)` markers, asset ids, versions, timestamps, thousands separators, trailing zeros) and on both ends of a range, since the unit sits only on the far end. Plus the placement: an agent with a `tool` is exempt because a tool is a live evidence source, an agent without one is checked, and an ungrounded figure warns the reviewer rather than failing the run |
...
| `test_kb_retrieval.py` | The KB retrieve Lambda, and specifically the asymmetry between its two filters — which is a security boundary, not a feature. The filter an AGENT sends is one scalar, its corpus, because the generated Cedar permit is a scalar value match evaluated at the Gateway before this function runs; the rich multi-condition filter is set on the TARGET, invisible to the caller, and ANDed with the agent's so it can only narrow. So these assert that the target filter applies when the agent sends nothing, that the two intersect rather than one replacing the other, that a caller who sends a whole filter document has it treated as the corpus NAME it claims to be, and that a malformed target filter refuses to retrieve rather than quietly dropping the scope. Plus the tuning knobs, and the absence of `searchType` — a knob that was added, deployed, and removed the same day because S3 Vectors refuses hybrid search |
| `test_lifecycle_research.py` | The shipped `type: "openapi"` demo, and specifically the two ways its date reasoning was WRONG the first time it ran against the real API. The catalogue returns releases NEWEST first and the agent assumed oldest, so "the newest supported releases" named the oldest four and "the most recent release to reach end of life" named Python 2.6, ended 2013. And `isMaintained` marks the maintenance TRACK rather than current support — it is true for Node 12, whose support ended in 2022 — so reading it as support claimed eight supported Node releases and offered a date four years past as "the earliest planning deadline". Neither produced a type error, a contract violation or an empty result: the asset was well-formed, every date in it was real, and the sentences built from them were false. Fixtures are the real response shape, because a fixture invented from the schema would have shown neither |
