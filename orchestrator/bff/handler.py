...
def _run(event: dict) -> dict:
    ...
    try:
        agentcore.invoke_agent_runtime(
            ...
            runtimeSessionId=session_id.ljust(33, "0")[:33],
            payload=json.dumps(payload).encode())
...
def _user(event: dict) -> str:
    """Authenticated user from the Cognito JWT claims (empty when auth disabled)."""
...
# --- Observability reads ---------------------------------------------------
# Cost is computed and stored per row by the runtime (app/observability), so the
# BFF only SUMS pre-computed numbers here — no pricing logic lives in the BFF.
...
def _api(event: dict, context) -> dict:
    ...
    if method == "POST" and path == "/api/sessions":
        topic = body.get("topic", "Design a serverless data pipeline on AWS")
